...
- **The `wa_id` slot is not a phone number.** WhatsApp lets a user hide their number; those contacts have `phone: null` and are identified by a BSUID (`XX.<alnum>`) plus, almost always, a username. The API resolves all three in the same field, and neither this server nor the SDK transforms the value — so **the description IS the contract with the model**. Never write a `wa_id` description that only mentions E.164: import `WA_ID_DESCRIPTION` / `CONTACT_IDENTIFIER_DESCRIPTION` from `src/lib/recipient.ts` instead. `tests/unit/tools/recipient-identifiers.test.ts` walks the real tool list and fails if a new tool with a `wa_id` slot forgets this.
- Related: don't make a "recipient kind" discriminator required (`send_template.contact_type` is optional on purpose). Forcing the model to declare a kind is what made hidden-number contacts unreachable — omitting it lets the API classify each recipient by format.
