...
Run before publishing each release of the `.mcpb`.

## Build
1. From repo root: `npm run package:dxt`
2. Expect: clean exit, prints `Done. release/wasapi-mcp-<version>.mcpb (X.XX MB)`

## Archive inspection
3. `unzip -l release/wasapi-mcp-*.mcpb | head -30`
...
5. `unzip -p release/wasapi-mcp-*.mcpb manifest.json | jq .version` must match `package.json#version`.

## Claude Desktop install (macOS)
6. Double-click `release/wasapi-mcp-<version>.mcpb`.
