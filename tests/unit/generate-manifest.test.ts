...
const pkgFixture = {
  ...
  repository: { type: "git", url: "git+https://github.com/Vinix-Code-Dev/wasapi-mcp-server.git" },
  license: "ISC",
};
...
describe("buildManifest", () => {
  ...
  it("derives homepage/documentation/support from repository URL", () => {
    const m = buildManifest(pkgFixture);
    expect(m.homepage).toBe("https://github.com/Vinix-Code-Dev/wasapi-mcp-server");
    expect(m.documentation).toBe("https://github.com/Vinix-Code-Dev/wasapi-mcp-server#readme");
    expect(m.support).toBe("https://github.com/Vinix-Code-Dev/wasapi-mcp-server/issues");
  });

  it("normalizes SCP-style SSH repository URLs (git@host:user/repo.git)", () => {
    const m = buildManifest({
      ...pkgFixture,
      repository: { type: "git", url: "git@github.com:Vinix-Code-Dev/wasapi-mcp-server.git" },
    });
    expect(m.homepage).toBe("https://github.com/Vinix-Code-Dev/wasapi-mcp-server");
    expect(() => manifestSchema.parse(m)).not.toThrow();
  });
  ...
  it("normalizes ssh:// repository URLs", () => {
    const m = buildManifest({
      ...pkgFixture,
      repository: { type: "git", url: "ssh://git@github.com/Vinix-Code-Dev/wasapi-mcp-server.git" },
    });
    expect(m.homepage).toBe("https://github.com/Vinix-Code-Dev/wasapi-mcp-server");
  });
